...
def draw_feature(ax, eopatch: EOPatch, time_idx: Union[List[int], int, None], feature: tuple, grid: bool = True, band: int = None, interpolation: str = 'none',
    ...
    def _show_single_ts(axis, img, ts):
        print(img.shape, get_extent(eopatch), 'ai ai')
...
def load_tiffs(datapath: str, 
    ...
    try:
        ...
        if "SEN3" in str(datapath):
            eop.meta_info["day"] = np.array([int(f[-12:-8]) for f in tiles])
        else:
            eop.meta_info["day"] = np.array([int(re.search(r"day\d\d\d\d", f)[0][3:]) for f in tiles])
    except:
        pass
